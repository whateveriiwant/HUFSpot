...
const BASE_HEADERS: Record<string, string> = {
    ...
    'Referer': 'https://wis.hufs.ac.kr/src08/jsp/lecture/LECTURE2020L.jsp?type=plan&tab_lang=K',
...
function parseTimeLocation(str: string, campus: string): ScheduleEntry[] {
    ...
    const normalized = str.replace(/\b(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\b/g, m => EN_TO_KR[m] ?? m);
...
async function run() {
    ...
    console.log(`📋 총 ${targetMajors.length}개 전공 (서울 ${h1Count}개 + 글로벌 ${h2Count}개)`);
    ...
    try {
        ...
        for (const [index, major] of targetMajors.entries()) {
            console.log(`[${index + 1}/${targetMajors.length}] [${major.campus}] ${major.name}`);
            ...
            for (const lecture of lectures) {
                const timeStr: string = lecture.dayTimeDisplayE || lecture.dayTimeDisplay || '';
                if (!timeStr) continue;
